                  ...
                  const wasm_extern_vec_t *imports, own wasm_trap_t **traps)
{
    return wasm_instance_new_with_args(store, module, imports, traps,
                                       KILOBYTE(32), KILOBYTE(32), NULL);
}

wasm_instance_t *
wasm_instance_new_with_lb_alloc(
    wasm_store_t *store, const wasm_module_t *module,
    const wasm_extern_vec_t *imports, own wasm_trap_t **traps,
    const wasm_linear_buffer_alloc_t *lb_alloc)
{
    return wasm_instance_new_with_args(store, module, imports, traps,
                                       KILOBYTE(32), KILOBYTE(32), lb_alloc);
}
                            ...
                            const wasm_linear_buffer_alloc_t *lb_alloc)
{
    ...
    instance->inst_comm_rt = wasm_runtime_instantiate(
        *module, stack_size, heap_size, lb_alloc, error_buf, sizeof(error_buf));
